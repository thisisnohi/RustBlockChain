...
pub async fn all_nodes() -> Result<HttpResponse, MyError> {
    info!("获取所有链接的节点...{}", "".to_string());
    let mut app_state = GLOBAL_APP_STATE.lock().unwrap().clone();
    let node_list = app_state.node_list.lock().unwrap().clone();
    Ok(HttpResponse::Ok().json(node_list))
}

/// 添加节点
pub async fn add_node(node_form: web::Json<NodeForm>) -> Result<HttpResponse, MyError> {
